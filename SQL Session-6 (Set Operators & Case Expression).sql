...
-- 2 g�nden ge� kargolanan sipari�lerin haftan�n g�nlerine g�re da��l�m�n� hesaplay�n�z.
	...
	select c.customer_id, c.first_name, c.last_name, o.order_id,
				case when cat.category_name='speakers' then 1 else 0 end as spe,
				case when cat.category_name='Computer Accessories' then 1 else 0 end as ca,
				case when cat.category_name='mp4 player' then 1 else 0 end as mp4
	from sale.customer c
	inner join sale.orders o on c.customer_id=o.customer_id
	inner join sale.order_item oi on o.order_id=oi.order_id
	inner join product.product p on oi.product_id=p.product_id
